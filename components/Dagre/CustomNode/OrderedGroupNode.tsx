...
export default function OrderedGroupNode({ id, data, isConnectable }: { id: string, data: { label: string }, isConnectable: boolean }) {
    ...
    return (
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', width: groupSettings.width }}>
            <div>
                ...
                <div style={{ backgroundColor: 'red' }}>
                    {member.map((value, index) => {
                        return <div style={{ backgroundColor: '#ffe5d1', marginBottom: `${index !== member.length - 1 ? defaultSettings.Margin + 'px' : ''}`, width: `${defaultSettings.singleWidth}px`, height: `${defaultSettings.singleHeight}px` }} key={value.id}> {value.name}</div>
                    })}
                </div>
