        ...
        <div style={{ backgroundColor: 'blue', display: 'flex', flexDirection: 'row', alignItems: 'center', width: 'fit-content' }}>
            ...
            <div style={{backgroundColor: 'red', display: 'flex', flexDirection: 'row'}}>
                {member.map((value, index) => {
                    return <div style={{ backgroundColor: '#ffabdc', marginRight: `${index !== member.length - 1 ? defaultSettings.Margin + 'px' : ''}`, width: `${defaultSettings.singleWidth}px`, height: `${defaultSettings.singleHeight}px` }} key={value.id}> {value.name}</div>
                })}
            </div>
